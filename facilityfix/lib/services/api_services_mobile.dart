...
class APIService {
  ...
  /// Return inventory request back to stock
  Future<Map<String, dynamic>> returnInventoryRequest(
    String requestId, {
    int? quantity,
  }) async {
    try {
      ...
      if (quantity != null) queryParams.add('quantity=$quantity');
      final query = queryParams.isNotEmpty ? '?${queryParams.join('&')}' : '';

      final response = await post(
        '/inventory/requests/$requestId/return$query',
        headers: _authHeaders(token),
        body: jsonEncode({}),
      );
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        final errorBody = _tryDecode(response.body);
        throw Exception(
          'Failed to return inventory request: ${errorBody['detail'] ?? response.body}',
        );
      }
    } catch (e) {
      print('Error returning inventory request: $e');
      rethrow;
    }
  }

  /// Mark inventory request as received
  Future<Map<String, dynamic>> receiveInventoryRequest(
    String requestId, {
    bool deductStock = false,
  }) async {
    try {
      ...
      final query = '?deduct_stock=$deductStock';

      final response = await post(
        '/inventory/requests/$requestId/receive$query',
        headers: _authHeaders(token),
        body: jsonEncode({}),
      );
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        final errorBody = _tryDecode(response.body);
        throw Exception(
          'Failed to receive inventory request: ${errorBody['detail'] ?? response.body}',
        );
      }
    } catch (e) {
      print('Error receiving inventory request: $e');
      rethrow;
    }
  }
  ...
  /// Handle reservation actions (request/return) using the new flexible endpoint
  Future<Map<String, dynamic>> handleReservationAction({
    ...
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        final errorBody = _tryDecode(response.body);
        throw Exception(
          'Failed to handle reservation action: ${errorBody['detail'] ?? response.body}',
        );
      }
    } catch (e) {
  ...
  Future<Map<String, dynamic>> returnReservation(
    String reservationId, {
    int? quantity,
  }) async {
    try {
      ...
      final response = await post(
        '/inventory/reservations/$reservationId/return$query',
        headers: _authHeaders(token),
        body: jsonEncode({}),
      );
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        final errorBody = _tryDecode(response.body);
        throw Exception(
          'Failed to return reservation: ${errorBody['detail'] ?? response.body}',
        );
      }
    } catch (e) {
      print('Error returning reservation: $e');
      rethrow;
    }
  }

  /// Create inventory request from reservation
  Future<Map<String, dynamic>> createRequestFromReservation(
    String reservationId, {
    String purpose = "Maintenance task",
  }) async {
    try {
      await _refreshRoleLabelFromToken();
      final token = await _requireToken();
      ...
      final queryParams = <String>[];
      queryParams.add('purpose=${Uri.encodeComponent(purpose)}');
      final query = '?${queryParams.join('&')}';

      final response = await post(
        '/inventory/reservations/$reservationId/request$query',
        headers: _authHeaders(token),
        body: jsonEncode({}),
      );
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        final errorBody = _tryDecode(response.body);
        throw Exception(
          'Failed to create request from reservation: ${errorBody['detail'] ?? response.body}',
        );
      }
    } catch (e) {
      print('Error creating request from reservation: $e');
      rethrow;
    }
  }
  ...
  /// Return received inventory items (staff returns unused/excess items)
  /// This method is deprecated, use returnReservation() instead
  @deprecated
  Future<Map<String, dynamic>> returnInventoryItems(
    String reservationId, {
    int? returnQuantity,
    String? reason,
  }) async {
    // Use the new standardized method
    return await returnReservation(reservationId, quantity: returnQuantity);
  }

  /// Request replacement for defective/broken items
  Future<Map<String, dynamic>> requestInventoryReplacement(
    String reservationId,
    ...
    try {
      ...
      print('[API] Error requesting inventory replacement: $e');
      return {'success': false, 'error': e.toString()};
    }
  }

  /// Get reservations assigned to current staff member
  Future<Map<String, dynamic>> getMyInventoryReservations() async {
    await _refreshRoleLabelFromToken();
    _logReq('GET', '/inventory/reservations');
  ...
  /// Mark inventory items as consumed/used during maintenance
  Future<Map<String, dynamic>> consumeInventoryReservation(
    String reservationId,
    int consumedQuantity,
    String notes,
  ) async {
    await _refreshRoleLabelFromToken();
    _logReq('PUT', '/inventory/reservations/$reservationId/consumed');
    ...
    try {
      ...
      print('[API] Error consuming inventory reservation: $e');
      return {'success': false, 'error': e.toString()};
    }
  }
