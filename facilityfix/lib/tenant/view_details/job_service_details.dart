...
class _TenantJobServiceDetailPageState
  ...
  Future<void> _enrichWithUserNames(
    ...
    try {
      ...
      // Fetch assessed_by name if we have the ID but not the name
      if (data.containsKey('assessed_by') &&
          data['assessed_by'] != null &&
          !data.containsKey('assessed_by_name')) {
        final userId = data['assessed_by'].toString();
        print('[DEBUG] Fetching user name for assessed_by: $userId');
        final userData = await apiService.getUserById(userId);
        if (userData != null) {
          final firstName = userData['first_name'] ?? '';
          final lastName = userData['last_name'] ?? '';
          data['assessed_by_name'] = '$firstName $lastName'.trim();
          print('[DEBUG] Set assessed_by_name to: ${data['assessed_by_name']}');
        }
      }
    } catch (e) {
  ...
  /// Returns the schedule_availability string directly without parsing
                  ...
                  child: Column(
                    children: [
                      ...
                      if (_jobServiceData != null) ...[
                        ...
                        // Completion Assessment box (matching staff version)
                          ...
                          Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                if (_jobServiceData!['assessed_at'] !=
                                    null) ...[
