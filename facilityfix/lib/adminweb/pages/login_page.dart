    ...
    Future<void> _login() async {
      ...
      try {
        final res = await api.loginRoleBased(
          role: 'admin', // backend can ignore; mobile sends it too
          email: email,
          password: pass,
        );

        // Align with mobile: read snake_case (with fallbacks)
        final Map<String, dynamic> profile =
            (res['profile'] is Map<String, dynamic>) ? (res['profile'] as Map<String, dynamic>) : <String, dynamic>{};

        final role = ((res['role'] ?? profile['role'] ?? 'admin').toString()).toLowerCase();
        ...
        final localProfile = <String, dynamic>{
          ...
          'staff_department': profile['staff_department'], // staff-only field; harmless for admin
        };
        ...
        if (!mounted) return;
        _showSnack('Welcome ${localProfile['full_name'] ?? localProfile['email'] ?? 'Admin'}');
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (_) => const AdminWebDashPage()),
        );
      } catch (e) {
              ...
              child: Center(
                child: SizedBox(
                  ...
                  child: Column(
                    ...
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Welcome to FacilityFix',
                      ...
                      Row(
                        ...
                        children: [
                          ...
                          TextButton(
                            onPressed: () {
                              _showSnack('Forgot Password flow not implemented on web yet.');
                            },
