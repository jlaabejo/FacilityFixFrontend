...
class _InternalMaintenanceFormPageState
  ...
  final _otherLocationController = TextEditingController(); // For custom location input

  // -------------------- STATE --------------------
  String? _selectedPriority;
  // Status removed - automatically set to "New"
  ...
  void _handleRecurrenceChange(String? value) {
    ...
    setState(() {
      ...
      _updateNotifications(); // Auto-update notifications based on recurrence
    });
  }
  ...
  void _updateNotifications() {
    ...
    setState(() {
      if (notifications.isNotEmpty) {
        _selectedAdminNotification = notifications.join(', ');
        _selectedStaffNotification = notifications.join(', ');
      }
    });
  }
  
  // Handle department change to auto-assign staff
  void _handleDepartmentChange(String? department) {
    setState(() {
      _selectedDepartment = department;
      
      // Only auto-assign if staff field is empty
      if (department != null && _staffMembers.isNotEmpty && _assignedStaffController.text.trim().isEmpty) {
        final departmentStaff = _staffMembers.where((staff) {
          final staffDept = staff['department']?.toString().toLowerCase() ?? '';
          return staffDept == department.toLowerCase();
        }).toList();
        
        if (departmentStaff.isNotEmpty) {
          // Auto-select the first staff member from the department
          final selectedStaff = departmentStaff.first;
          _selectedStaffUserId = selectedStaff['uid'] ?? selectedStaff['id'];
          
          final firstName = selectedStaff['first_name'] ?? '';
          final lastName = selectedStaff['last_name'] ?? '';
          _assignedStaffController.text = '$firstName $lastName'.trim();
        }
  ...
  void dispose() {
    ...
    _otherLocationController.dispose(); // Dispose other location controller
    super.dispose();
  }
  ...
  Future<void> _onNext() async {
    ...
    final maintenance = <String, dynamic>{
      ...
      'category': 'preventive',
      'task_type': 'internal', // Explicitly set as internal
      'recurrence_type':
          _selectedRecurrence != null && _selectedRecurrence != 'None'
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Divider(color: Color(0xFFE2E8F0), height: 1, thickness: 1),
                      const SizedBox(height: 32),
                      // ===== Checklist & Schedule Section =====
                      _buildSectionHeader(
                        'Checklist & Schedule',
                        'Add step-by-step tasks and set maintenance schedule',
                      ),
                      const SizedBox(height: 24),
                                    ...
                                    Expanded(
                                      child: _fieldBox(
                                        child: TextFormField(
                                          ...
                                          decoration: _decoration('Add a task step...'),
                      ...
                      // Display added checklist items - Left side only
                      ...
                      _buildSectionHeader(
                        'Assign Staff',
                        'Select department first to automatically assign available staff',
                      ),
                      const SizedBox(height: 24),
                                  ...
                                  child: Autocomplete<Map<String, dynamic>>(
                                    optionsBuilder: (textEditingValue) {
                                      if (textEditingValue.text.isEmpty) {
                                        // Filter by selected department if available
                                      ...
                                      // Filter by department AND search text
                                      return _staffMembers.where((staff) {
                                        final name =
                                        ...
                                        if (_selectedDepartment != null) {
                                          final staffDept = staff['department']?.toString().toLowerCase() ?? '';
                                          return matchesSearch && staffDept == _selectedDepartment!.toLowerCase();
                                        }
                                    ...
                                    fieldViewBuilder: (
                                      ...
                                      if (_assignedStaffController.text.isNotEmpty &&
                                          controller.text != _assignedStaffController.text) {
                                        controller.text = _assignedStaffController.text;
                                      }
                                      ...
                                      return TextFormField(
                                        ...
                                        decoration: _decoration(
                                          _selectedDepartment != null 
                                            ? (_assignedStaffController.text.isEmpty 
                                                ? 'Will auto-assign from ${_selectedDepartment}...'
                                                : 'Type to change staff...')
                                        ...
                                        onChanged: (value) {
                                          ...
                                          if (value != _assignedStaffController.text) {
                                            setState(() {
                                              _selectedStaffUserId = null;
                                            });
                                          }
                      ...
                      _buildSectionHeader(
                        'Inventory Items',
                        'Request materials and tools needed for this maintenance task',
                      ),
                      const SizedBox(height: 24),

                      // Inventory selection with Add Item button beside it - Left side only
                      Row(
                        children: [
                          Expanded(
                            flex: 6,
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                _fieldLabel('Select Inventory Item'),
                                Row(
                                  children: [
                                    Expanded(
                                      child: _fieldBox(
                                        child: Container(
                                          ...
                                          child: Text(
                                            'Click "Add Item" to select inventory...',
                      ...
                      // Inventory items list - Left side only
                      _fieldLabel('Inventory List'),
                      Row(
                        children: [
                          Expanded(
                            flex: 6,
                            child: _selectedInventoryItems.isEmpty
                                ? Container(
                                    width: double.infinity,
                                    padding: const EdgeInsets.all(16),
                                    decoration: BoxDecoration(
                      ...
                      _buildSectionHeader(
                        'Remarks / Admin Notes',
                        'Add any additional notes or special instructions for this task',
                      ),
                      const SizedBox(height: 24),
                      _fieldLabel('Add Admin Note'),
                      TextFormField(
                        controller: _remarksController,
                        maxLines: 5,
                        decoration: _decoration('Enter Admin Notes....'),
                      ),
                      const SizedBox(height: 32),
  ...
  InputDecoration _decoration(String hint) => InputDecoration(
    hintText: hint,
    hintStyle: TextStyle(color: Colors.grey[240]),
    isDense: true,
