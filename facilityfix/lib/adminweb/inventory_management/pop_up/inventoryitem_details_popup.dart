...
class _InventoryItemDetailsContentState extends State<_InventoryItemDetailsContent> {
  ...
  Future<void> _maybeLoadReservedStock() async {
    try {
      // Prefer the database document id when matching reservations
      final id = _itemData['id'] ?? _itemData['_doc_id'] ?? _itemData['inventory_id'] ?? _itemData['itemCode'] ?? _itemData['item_code'];
