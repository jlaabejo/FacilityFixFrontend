...
class ApiService {
  ...
  Future<List<Map<String, dynamic>>> listTaskTypes({
    bool includeInactive = false,
  }) async {
    try {
      final headers = await _getAuthHeaders();
      final response = await http.get(
        Uri.parse('$baseUrl/task-types?include_inactive=$includeInactive'),
        headers: headers,
      );
      if (response.statusCode >= 200 && response.statusCode < 300) {
        final decoded = json.decode(response.body);
        ...
        throw Exception('Unexpected task types response: ${response.body}');
      }
      throw Exception(
        'Failed to load task types: ${response.statusCode} ${response.body}',
      );
    } catch (e) {
      print('[v0] Error listing task types: $e');
      rethrow;
    }
  }

  Future<Map<String, dynamic>> getTaskType(String taskTypeId) async {
    try {
      final headers = await _getAuthHeaders();
      final response = await http.get(
        Uri.parse('$baseUrl/task-types/$taskTypeId'),
        headers: headers,
      );
      if (response.statusCode >= 200 && response.statusCode < 300) {
        final decoded = json.decode(response.body);
        ...
        throw Exception('Unexpected task type response: ${response.body}');
      }
      throw Exception(
        'Failed to load task type: ${response.statusCode} ${response.body}',
      );
    } catch (e) {
      print('[v0] Error fetching task type $taskTypeId: $e');
      rethrow;
    }
  }

  Future<Map<String, dynamic>> createTaskType(
    Map<String, dynamic> payload,
  ) async {
    try {
      final headers = await _getAuthHeaders();
      final response = await http.post(
        Uri.parse('$baseUrl/task-types'),
        headers: headers,
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        throw Exception(
          'Unexpected create task type response: ${response.body}',
        );
      }
      throw Exception(
        'Failed to create task type: ${response.statusCode} ${response.body}',
      );
    } catch (e) {
      print('[v0] Error creating task type: $e');
      rethrow;
    }
  }

  Future<Map<String, dynamic>> updateTaskType(
    String taskTypeId,
    Map<String, dynamic> payload,
  ) async {
    try {
      ...
      if (response.statusCode >= 200 && response.statusCode < 300) {
        ...
        throw Exception(
          'Unexpected update task type response: ${response.body}',
        );
      }
      throw Exception(
        'Failed to update task type: ${response.statusCode} ${response.body}',
      );
    } catch (e) {
      print('[v0] Error updating task type $taskTypeId: $e');
      rethrow;
    }
  }
  ...
  Future<void> deleteTaskType(String taskTypeId) async {
    try {
      ...
      throw Exception(
        'Failed to delete task type: ${response.statusCode} ${response.body}',
      );
    } catch (e) {
      print('[v0] Error deleting task type $taskTypeId: $e');
      rethrow;
    }
  }
  ...
  /// Retrieve inventory forecasting data for a building
  Future<List<Map<String, dynamic>>> getInventoryForecasting(
    String buildingId,
  ) async {
    final url = Uri.parse('$baseUrl/inventory/forecasting/$buildingId');
    final headers = await _getAuthHeaders();
    try {
      ...
      print('[ApiService] Error fetching forecasting data: $e');
      return [];
    }
  }
  ...
  // ============================================
  // TASK TYPE DROPDOWN ENDPOINTS
  // ============================================

  /// Get task types for dropdown (simplified format)
  Future<List<Map<String, dynamic>>> getTaskTypesForDropdown() async {
    try {
      final headers = await _getAuthHeaders();
      final response = await http.get(
        Uri.parse('$baseUrl/maintenance/task-types'),
        headers: headers,
      );

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        if (data['success'] == true && data['task_types'] != null) {
          return List<Map<String, dynamic>>.from(data['task_types']);
        }
      }
      return [];
    } catch (e) {
      print('[API] Error fetching task types for dropdown: $e');
      return [];
    }
  }

  /// Get inventory items associated with a specific task type
  Future<List<Map<String, dynamic>>> getTaskTypeInventoryItems(
    String taskTypeId,
  ) async {
    try {
      final headers = await _getAuthHeaders();
      final response = await http.get(
        Uri.parse('$baseUrl/maintenance/task-types/$taskTypeId/inventory'),
        headers: headers,
      );

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        if (data['success'] == true && data['inventory_items'] != null) {
          return List<Map<String, dynamic>>.from(data['inventory_items']);
        }
      }
      return [];
    } catch (e) {
      print('[API] Error fetching task type inventory items: $e');
      return [];
    }
  }
  ...
  /// Create inventory reservations for maintenance task
  Future<Map<String, dynamic>> createInventoryReservations(
    ...
    try {
      ...
      throw Exception('Failed to create reservations');
    } catch (e) {
      print('[API] Error creating inventory reservations: $e');
      throw Exception('Reservation failed: $e');
    }
  }

  /// Get reservations for a maintenance task
  Future<List<Map<String, dynamic>>> getTaskReservations(String taskId) async {
    try {
      final headers = await _getAuthHeaders();
      final response = await http.get(
        Uri.parse('$baseUrl/inventory/reservations?task_id=$taskId'),
        headers: headers,
      );
      ...
      print('[API] Error fetching task reservations: $e');
      return [];
    }
  }
}
