...
class RoundRobinAssignmentService {
  ...
  Future<Map<String, dynamic>?> getNextStaffForDepartment(
    String department, {
    String? schedule,
  }) async {
    try {
      ...
      print(
        '[RoundRobin] Staff: ${selectedStaff['first_name']} ${selectedStaff['last_name']}',
      );

      return selectedStaff;
    } catch (e) {
      print(
        '[RoundRobin] Error getting next staff for department $department: $e',
      );
      return null;
    }
  }
  ...
  Future<Map<String, dynamic>?> autoAssignTask({
    ...
    required String
    taskType, // 'concern_slip', 'job_service', 'work_order', 'maintenance'
    ...
    try {
      ...
      final staff = await getNextStaffForDepartment(
        department,
        schedule: schedule,
      );
      ...
      // Extract staff ID from multiple possible field names
      final staffId = _extractStaffId(staff);
      if (staffId == null || staffId.toString().isEmpty) {
        print('[RoundRobin] Staff member has no valid ID');
        print('[RoundRobin] Staff object: $staff');
        return null;
      }
      ...
      try {
        ...
        print(
          '[RoundRobin] Successfully auto-assigned task $taskId to staff $staffId',
        );
        return staff;
      } catch (e) {
  ...
  /// Extract staff ID from staff object - tries multiple field names
  /// Checks: user_id, id, staff_id, _id, emp_id in order of priority
  dynamic _extractStaffId(Map<String, dynamic> staff) {
    // Priority order for field names that might contain staff ID
    const fieldPriority = [
      'user_id', // Primary field
      'id', // Fallback
      'staff_id', // Alternative naming
      '_id', // MongoDB-style ID
      'emp_id', // Employee ID
      'staff_user_id', // Compound field name
    ];

    for (final field in fieldPriority) {
      final value = staff[field];
      if (value != null && value.toString().isNotEmpty) {
        print('[RoundRobin] Extracted staff ID from field "$field": $value');
        return value;
      }
    }
  ...
  Future<void> _incrementDepartmentPointer(
    String department,
    int staffCount,
  ) async {
    try {
      ...
      print(
        '[RoundRobin] Updated pointer for $department: $currentPointer -> $newPointer',
      );
    } catch (e) {
  ...
  Future<void> resetAllPointers() async {
    try {
      ...
      final keys = prefs.getKeys().where(
        (key) => key.startsWith(_pointerKeyPrefix),
      );
