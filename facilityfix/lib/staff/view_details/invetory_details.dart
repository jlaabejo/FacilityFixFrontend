...
class _InventoryDetailsState extends State<InventoryDetails> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (widget.selectedTabLabel.toLowerCase() == 'inventory details' &&
          widget.itemId != null) {
      ...
      } else if (widget.selectedTabLabel.toLowerCase() == 'inventory request' &&
          widget.requestId != null) {
        // Fetch inventory request
        print('DEBUG: Fetching inventory request with ID: ${widget.requestId}');
        final data = await _apiService.getInventoryRequestById(
          widget.requestId!,
        );
        ...
        if (data == null) {
          ...
          // Backend now provides requested_by_name, no enrichment needed
          print(
            'DEBUG: Using requested_by_name from backend: ${data['requested_by_name']}',
          );
                    ...
                    child: AbsorbPointer(
                      child: TextFormField(
                        ...
                        controller: TextEditingController(
                          text:
                              neededDate == null ? '' : formatDate(neededDate!),
                        ),
                      ),
                    ),
                  ),
                  ...
                  custom_buttons.FilledButton(
                    ...
                    onPressed: () async {
                      ...
                      try {
                        ...
                        final profile =
                            await _apiService.fetchCurrentUserProfile();
                        final buildingId =
                            (profile != null && profile['building_id'] != null)
  ...
  Future<void> _createQuickRequest() async {
    ...
    final itemId =
        (_itemData?['item_code'] ?? _itemData?['id'] ?? '').toString();
    ...
    final confirm = await showDialog<bool>(
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: Text(
              'Request 1 × $itemName and add it to Inventory Requests?',
            ),
            actions: [
              TextButton(
    ...
    try {
      ...
      final requestedBy =
          (profile != null &&
                  (profile['user_id'] ?? profile['uid'] ?? profile['id']) !=
                      null)
              ? (profile['user_id'] ?? profile['uid'] ?? profile['id'])
                  .toString()
              : 'unknown';
    ...
    switch (widget.selectedTabLabel.toLowerCase()) {
      case 'inventory details':
        ...
        return InventoryDetailsScreen(
          ...
          quantity:
              '${item['current_stock'] ?? 0} ${item['unit_of_measure'] ?? 'pcs'}',
          reorderLevel:
              '${item['reorder_level'] ?? 0} ${item['unit_of_measure'] ?? 'pcs'}',
      ...
      case 'inventory request':
        ...
        return InventoryDetailsScreen(
          ...
          itemId: request['item_code'] ?? 'N/A',
          status: (request['status'] ?? 'pending').toString().toUpperCase(),
          // Request Item
          requestId: request['formatted_id'] ?? request['id'] ?? 'Unknown ID',
          requestQuantity: (request['quantity_requested'] ?? 0).toString(),
          requestUnit: request['unit_of_measure'] ?? 'pcs',
          dateNeeded: formatDate(
            request['requested_date'] ?? request['created_at'],
          ),
          reqLocation: request['location'] ?? 'N/A',
          maintenanceId: request['maintenance_task_id'] ?? request['reference_id'],
          // Requestor - use proper backend field names
          staffName: request['requested_by_name'] ?? 
                     request['staff_name'] ?? 
                     request['requester_name'] ?? 
                     'Unknown',
          staffDepartment: request['department'] ?? 
  ...
  Widget _buildStickyRequestBar() {
    ...
    if (lowerTab == 'inventory details') {
      ...
      // Only show "Mark as Received" button if status is "approved"
      final status = (_requestData?['status'] ?? '').toString().toLowerCase();
      if (status != 'approved') {
  ...
  Widget build(BuildContext context) {
    ...
    final lowerTab = widget.selectedTabLabel.toLowerCase();
    final showSticky =
        lowerTab == 'inventory details' || lowerTab == 'inventory request';
